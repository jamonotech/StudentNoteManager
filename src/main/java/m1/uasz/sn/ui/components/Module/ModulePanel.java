...
public class ModulePanel extends RoundedSideBar {
    private ModuleService moduleService = new ModuleService();
    private DefaultTableModel model;

    public ModulePanel(MainFrame mainFrame) {
...
// Ajout de l'effet au survol de l'icône
        ajouterIcon.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                String[] champs = {"Code", "Nom", "Volume Horaire", "Coefficient", "Credits", "Matricule Responsable", "Formation"};
                JTextField[] textFields = new JTextField[champs.length];
                Formulaire11ChampsFrame form = new Formulaire11ChampsFrame("Ajouter module", "create", "modules", champs, textFields, 500, 500, new ComponentValidation(), new ComponentAction());
                form.addWindowListener(new java.awt.event.WindowAdapter() {
                    @Override
                    public void windowClosed(java.awt.event.WindowEvent e) {
                        chargerModules(); // Rafraîchir après modification
                    }
                });
            }
        });
        ...
        // ✅ Colonnes du tableau
        String[] columns = {
                "ID", "Code", "Nom", "Volume Horaire", "Coefficient", "Crédits", "Responsable", "Actions"
        };
        ...
        TablesRendererEditor usersEditor = new TablesRendererEditor("modules", new ActionHandler() {

            @Override
            public void onView(Object id, String callingComponent) {
                System.out.println("Voir utilisateur ID : " + id);
                Module module = moduleService.trouverModule((Long) id);
                JPanel detailsPanel = new DetailsModPanel(mainFrame, module);
            ...
            public void onEdit(Object id, String callingComponent) {
                ...
                Module user = moduleService.trouverModule((Long) id);
                // Mapper correctement les valeurs de l'utilisateur
                String[] champs = {"Code", "Nom", "Volume Horaire", "Coefficient", "Credits", "Matricule Responsable", "Formation"};
                JTextField[] textFields = new JTextField[champs.length];

                textFields[0] = new JTextField();
                textFields[0].setText(user.getCode());
                textFields[1] = new JTextField();
                textFields[1].setText(user.getNom());
                textFields[2] = new JTextField();
                textFields[2].setText(String.valueOf(user.getVolumeHoraire()));
                textFields[3] = new JTextField();
                textFields[3].setText(String.valueOf(user.getCoefficient()));
                textFields[4] = new JTextField();
                textFields[4].setText(String.valueOf(user.getCredits()));
                textFields[5] = new JTextField();
                textFields[5].setText(user.getEnseignantResponsable().getMatricule());
                textFields[6] = new JTextField();
                textFields[6].setText(user.getFormation().getNom());

                new Formulaire11ChampsFrame("Modifier module", "update", "modules", champs, textFields, 800, 570,
            ...
            public void onDelete(Object id, String callingComponent) {
                ...
                if (confirmation == JOptionPane.YES_OPTION) {
                    moduleService.supprimerModule(moduleService.trouverModule((Long) id));
